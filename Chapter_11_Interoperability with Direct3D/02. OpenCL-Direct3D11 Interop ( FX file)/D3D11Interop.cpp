...
ID3DX11Effect *g_pID3DX11Effect = nullptr;
ID3DX11EffectTechnique *g_pID3DX11EffectTechnique = nullptr;
ID3DX11EffectShaderResourceVariable *g_pID3DX11SRV = nullptr;
ID3DX11EffectConstantBuffer *g_pID3DX11ConstantBuffer = nullptr;
...
bool Initialize()
{
    // code
        // Direct 3D 11
    HRESULT hr;
    ID3DBlob *p_ErrorBlob;

        //Effect
    hr = D3DX11CompileEffectFromFile( L"display_shader/TextureEffect.fx", nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, 0, 0, g_pID3D11Device, &g_pID3DX11Effect, &p_ErrorBlob);
    if( FAILED( hr))
    {
        char str[4096];
        ComErrorDescriptionString( hr, str);
        std::cerr << "D3DX11CompileEffectFromFile() Failed: " << str << std::endl;
        if( p_ErrorBlob)
        {
            std::cerr << (char *)p_ErrorBlob->GetBufferPointer();
            p_ErrorBlob->Release();
        }
        return false;
    }
        ...
        // obtain technique
    g_pID3DX11EffectTechnique = g_pID3DX11Effect->GetTechniqueByName("Render");
    ...
    g_pID3DX11EffectTechnique->GetPassByIndex( 0)->GetDesc( &pass_desc);

    hr = g_pID3D11Device->CreateInputLayout( &d3d_input_layout_desc, 1, pass_desc.pIAInputSignature, pass_desc.IAInputSignatureSize, &g_pID3D11InputLayout);
    if( FAILED( hr))
    {
        ...
        // get pointers
    g_pID3DX11ConstantBuffer = g_pID3DX11Effect->GetConstantBufferByName( "ConstantBuffer");
    g_pID3DX11SRV = g_pID3DX11Effect->GetVariableByName("u_texture")->AsShaderResource();
    ...
    g_pID3DX11ConstantBuffer->SetConstantBuffer( g_pID3D11Buffer_ConstantBuffer);
    ...
    if( FAILED( hr))
    {
        ...
        // effect shader resource view
    g_pID3DX11SRV->SetResource( g_pID3D11ShaderResourceView);
...
void Render()
{
    ...
    g_pID3D11DeviceContext->IASetInputLayout( g_pID3D11InputLayout);
        ...
        // g_pID3DX11EffectTechnique->GetDesc( &tech_desc);
        g_pID3DX11EffectTechnique->GetPassByIndex( 0)->Apply(0, g_pID3D11DeviceContext);
...
void Uninitialize()
{
    ...
    COM_INTERFACE_SAFE_RELEASE( g_pID3DX11Effect);
    COM_INTERFACE_SAFE_RELEASE( g_pID3DX11EffectTechnique);
    COM_INTERFACE_SAFE_RELEASE( g_pID3DX11SRV);
    COM_INTERFACE_SAFE_RELEASE( g_pID3DX11ConstantBuffer);
