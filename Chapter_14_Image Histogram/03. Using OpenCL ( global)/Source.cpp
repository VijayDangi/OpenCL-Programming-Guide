...
cl_kernel fn_histogram_image_rgba_unorm8 = nullptr;
...
uint32_t *histogram_result = nullptr;
...
int main( int argc, char **argv)
{
    ...
    fn_histogram_image_rgba_unorm8 = clCreateKernel( ocl_program, "histogram_image_rgba_unorm8", &ocl_err);
    ...
    {
        std::cerr << "clCreateImage2D() Failed." <<  ocl_err << "\n";
        cleanup();
        return EXIT_FAILURE;
    }
    ...
    ocl_err = clSetKernelArg( fn_histogram_image_rgba_unorm8, 0, sizeof( cl_mem), &ocl_input_image);
    ocl_err |= clSetKernelArg( fn_histogram_image_rgba_unorm8, 1, sizeof( cl_sampler), &ocl_sampler);
    ocl_err |= clSetKernelArg( fn_histogram_image_rgba_unorm8, 2, sizeof( cl_mem), &ocl_histogram_buffer);
    if( ocl_err)
    {
    ...
    ocl_err = clEnqueueNDRangeKernel(
                ocl_command_queue,
                fn_histogram_image_rgba_unorm8,
                2, nullptr, global_work_size, nullptr, 0, nullptr, nullptr);
    if( ocl_err)
    {
...
void  cleanup()
{
    ...
    RELEASE_CL_OBJECT( fn_histogram_image_rgba_unorm8, clReleaseKernel);
