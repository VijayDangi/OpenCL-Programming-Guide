...
const int OUT_IMAGE_WIDTH = 257;
const int OUT_IMAGE_HEIGHT = 1024;
const int BIN_COUNT = 257;
...
cl_kernel fn_histogram_partial_image_rgba_fp = nullptr;
cl_kernel fn_histogram_sum_partial_results_fp = nullptr;
...
float *normalized_image_bits = nullptr;
...
int main( int argc, char **argv)
{
    ...
    float* NormalizeImage( uint8_t *image_bits, int image_width, int image_height, int channel);
    void  cleanup();
    ...
    fn_histogram_partial_image_rgba_fp = clCreateKernel( ocl_program, "histogram_partial_image_rgba_fp", &ocl_err);
    ...
    fn_histogram_sum_partial_results_fp = clCreateKernel( ocl_program, "histogram_sum_partial_results_fp", &ocl_err);
    if( ocl_err != CL_SUCCESS)
    {
        std::cerr << "clCreateKernel() Failed.";
        cleanup();
    ...
    normalized_image_bits = NormalizeImage( image_bits, image_width, image_height, 4);
    if( normalized_image_bits == nullptr)
    {
        std::cerr << "NormalizeImage() Failed." << std::endl;
        cleanup();
        return EXIT_FAILURE;
    }
    ...
    ocl_histogram_buffer = clCreateBuffer( ocl_context, CL_MEM_WRITE_ONLY, BIN_COUNT * 3 * sizeof( unsigned int), nullptr, &ocl_err);
    ...
    ocl_input_image = clCreateImage2D(
                        ...
                        &ocl_image_format,
                        image_width, image_height, 0, normalized_image_bits, &ocl_err
                    );
    ...
    clGetKernelWorkGroupInfo(
        fn_histogram_partial_image_rgba_fp,
    ...
    ocl_partial_histogram_buffer = clCreateBuffer(
                                        ocl_context,
                                        CL_MEM_READ_WRITE,
                                        num_groups * BIN_COUNT * 3 * sizeof( unsigned int), nullptr, &ocl_err);
    ...
    ocl_err = clSetKernelArg( fn_histogram_partial_image_rgba_fp, 0, sizeof( cl_mem), &ocl_input_image);
    ocl_err |= clSetKernelArg( fn_histogram_partial_image_rgba_fp, 1, sizeof( int), &num_pixels_per_work_item);
    ocl_err |= clSetKernelArg( fn_histogram_partial_image_rgba_fp, 2, sizeof( cl_mem), &ocl_partial_histogram_buffer);
    if( ocl_err)
    {
    ...
    ocl_err = clSetKernelArg( fn_histogram_sum_partial_results_fp, 0, sizeof( cl_mem), &ocl_partial_histogram_buffer);
    ocl_err |= clSetKernelArg( fn_histogram_sum_partial_results_fp, 1, sizeof( int), &num_groups);
    ocl_err |= clSetKernelArg( fn_histogram_sum_partial_results_fp, 2, sizeof( cl_mem), &ocl_histogram_buffer);
    if( ocl_err)
    {
        std::cerr << "clSetKernelArg() Failed." << ocl_err << "\n";
        cleanup();
    ...
    clGetKernelWorkGroupInfo( fn_histogram_sum_partial_results_fp, ocl_device, CL_KERNEL_WORK_GROUP_SIZE, sizeof( size_t), &workgroup_size, nullptr);
    std::cout << "CL_KERNEL_WORK_GROUP_SIZE: " << workgroup_size << "\n\n";
    if( workgroup_size < 256)
    {
    ...
    ocl_err = clEnqueueNDRangeKernel(
                ocl_command_queue,
                fn_histogram_sum_partial_results_fp,
    ...
    if( ocl_err)
    {
        ...
        // read the result
    histogram_result = new unsigned int[BIN_COUNT * 3];
    ...
    ocl_err = clEnqueueReadBuffer( ocl_command_queue, ocl_histogram_buffer, CL_TRUE, 0, BIN_COUNT * 3 * sizeof(unsigned int), histogram_result, 0, nullptr, nullptr);
    if( ocl_err)
    {
        ...
        /******** SAVE HISTOGRAM *******************/
    SaveHistogramGraphImage( histogram_result, histogram_result + 257 , histogram_result + 514 , b_save_filled_graph, b_save_separate_channel_graph);
...
void  cleanup()
{
    ...
    RELEASE_CL_OBJECT( fn_histogram_partial_image_rgba_fp, clReleaseKernel);
    RELEASE_CL_OBJECT( fn_histogram_sum_partial_results_fp, clReleaseKernel);
    ...
    RELEASE_CL_OBJECT( normalized_image_bits, delete);
}
...
/**
 * @brief NormalizeImage() : Convert images value [0, 255] to [ 0.0, 1.0]
 */
float* NormalizeImage( uint8_t *image_bits, int image_width, int image_height, int channel)
{
    // variable declaration
    float *ret_image_bits = nullptr;

    // code
    if( !image_bits || !image_width || !image_height)
    {
        return nullptr;
    }

    ret_image_bits = new float[ image_width * image_height * channel];

    for( int i = 0; i < image_width * image_height; ++i)
    {
        for( int j = 0; j < channel; ++j)
        {
            int index = channel * i + j;
            ret_image_bits[ index] = (float)image_bits[index] / 255.0f;
        }
    }
