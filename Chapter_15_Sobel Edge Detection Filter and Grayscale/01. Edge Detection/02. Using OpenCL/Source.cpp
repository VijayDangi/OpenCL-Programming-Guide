...
cl_kernel sobel_edge_detection = nullptr;
...
int main( int argc, char **argv)
{
    ...
    ocl_program = CreateProgram( ocl_context, ocl_device, "sobel_edge_detection.cl");
    ...
    sobel_edge_detection = clCreateKernel( ocl_program, "sobel_edge_detection", &ocl_err);
    ...
    std::cout << To_String( global_work_size[1]) << " : " << global_work_size[1] << "\n\n";
        ...
        //set kernel argument
    ocl_err = clSetKernelArg( sobel_edge_detection, 0, sizeof( cl_mem), &ocl_input_image);
    ocl_err |= clSetKernelArg( sobel_edge_detection, 1, sizeof( cl_mem), &ocl_output_image);
    ...
    ocl_err = clEnqueueNDRangeKernel( ocl_command_queue, sobel_edge_detection, 2, nullptr, global_work_size, nullptr, 0, nullptr, nullptr);
    if( ocl_err != CL_SUCCESS)
    {
...
void  cleanup()
{
    ...
    RELEASE_CL_OBJECT( sobel_edge_detection, clReleaseKernel);
