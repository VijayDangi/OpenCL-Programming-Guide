    ...
    for( int j = 0; j < image_height; ++j)
    {
        for( int i = 0; i < image_width; ++i)
        {
            ...
            */
            ...
            float gs_x = 0.3333f * (Gxx + Gxy + Gxz);
            float gs_y = 0.3333f * (Gyx + Gyy + Gyz);
            float g = sqrtf( gs_x * gs_x + gs_y * gs_y);

#endif
            output_image_bits[ 4 * ( j * image_width + i) + 0] = (uint8_t) (g * 255.0f);
            output_image_bits[ 4 * ( j * image_width + i) + 1] = (uint8_t) (g * 255.0f);
            output_image_bits[ 4 * ( j * image_width + i) + 2] = (uint8_t) (g * 255.0f);
