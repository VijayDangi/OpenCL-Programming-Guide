...
int main( int argc, char **argv)
{
    ...
    size_t global_work_size[] = { M_DIMENSION, N_DIMENSION};
    ocl_err = clEnqueueNDRangeKernel(
        ocl_command_queue, ocl_matrix_multiplication_kernel, 2, nullptr,
        global_work_size, nullptr, 0, nullptr, &profile_event
    );
    if( ocl_err != CL_SUCCESS)
    {
    ...
    for( i = 0; i < n_dim; ++i)
    {
        for( j = 0; j < m_dim; ++j)
        {
            ...
            for( k = 0; k < p_dim; ++k)
            {
                ...
                temp = temp + (*(A + (i * n_dim + k))) * (*(B + (k * p_dim + j)));
            }
            *(Out_C + ( i * n_dim + j)) = temp;
        }
    }
}
