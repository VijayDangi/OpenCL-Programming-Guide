...
int main( int argc, char **argv)
{
    ...
    if( ocl_program == nullptr)
    {
        ...
        Cleanup( ocl_context, ocl_command_queue, ocl_program, ocl_kernel);
        return 1;
    }

        // Create OpenCL Kernel
    ocl_kernel = clCreateKernel( ocl_program, "square", nullptr);
    ...
    cl_mem ocl_buffer = clCreateBuffer( ocl_context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, sizeof( cl_int) * NUM_BUFFER_ELEMENTS * num_devices, inputOutput, nullptr);
    if( ocl_buffer == nullptr)
    {
        delete inputOutput;
        delete ocl_devices;
    ...
    ocl_err = clSetKernelArg( ocl_kernel, 0 /* argument index */, sizeof( cl_mem), &ocl_buffer);
    if( ocl_err != CL_SUCCESS)
    {
    ...
    std::vector<cl_event> events;
        ...
        // Create a command-queue for each devices
    for( int i = 0; i < num_devices; ++i)
    {
        cl_command_queue ocl_command_queue = clCreateCommandQueue( ocl_context, ocl_devices[i], 0, nullptr);
        if( ocl_command_queue == nullptr)
        {
            std::cerr << "CreateCommandQueue() Failed." << __LINE__ << std::endl;
    ...
    for( int i = 0; i < queue.size(); ++i)
    {
        ...
        ocl_err = clEnqueueNDRangeKernel( queue[i], ocl_kernel, 1, (const size_t*) &offset, (const size_t*) &gWI, nullptr, 0, nullptr, &event);
        if( ocl_err != CL_SUCCESS)
        {
    ...
    for( int i = 0; i < queue.size(); ++i)
    {
        size_t offset = i * NUM_BUFFER_ELEMENTS;
        ocl_err = clEnqueueReadBuffer( queue[i], ocl_buffer, CL_TRUE, 0, NUM_BUFFER_ELEMENTS * sizeof( float), inputOutput + offset, 0, nullptr, nullptr);
        if( ocl_err != CL_SUCCESS)
        {
